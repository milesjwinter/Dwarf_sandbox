...
#Lum_bins = np.logspace(31.0,36.0,50)
Lum_bins = np.logspace(31.5,35.5,80)
...
Th_mc = np.random.uniform(0.0,2.0*np.pi,size=NMSP)

completeness = np.zeros((NMC,len(Lum_bins)))

for q in xrange(NMC):
    #Generate random z coordinates
    Z_int_vals = np.random.uniform(0.0,Hz[q],size=NMSP)
    Z_sign = (-1.0)**np.random.randint(2,size=NMSP)  
    Z_mc = Z_sign*Hz[q]*np.log(Hz[q]/(Hz[q]-Z_int_vals))
    
    #Generate random r coordinates
    R_int_vals = np.random.uniform(0.0,Hr[q],size=NMSP)
    R_mc = Hr[q]*np.log(Hr[q]/(Hr[q]-R_int_vals))
...
np.savetxt('comp_unc_upper.txt',np.transpose(comp_unc))

#save completeness array to file
#np.savetxt('comp_array.txt',completeness)
